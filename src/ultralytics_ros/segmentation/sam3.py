...
class SAM3(rclpy.node.Node):
    ...
    def __init__(self) -> None:
        ...
        self.model = ultralytics.models.sam.SAM3SemanticPredictor(
            overrides={
                "task": "segment",
                "mode": "predict",
                "model": model_file,
        ...
        self.result_queue: queue.Queue[SAM3.Result] = queue.Queue()
    ...
    def _build_and_publish_result(self) -> None:
        while rclpy.ok():
            ...
            detections.detections = self._build_detections(result.results.boxes)
            ...
            typing.cast(list, detection.results).append(
                vision_msgs.msg.ObjectHypothesisWithPose(
                    hypothesis=vision_msgs.msg.ObjectHypothesis(
                        class_id=self.text_prompts[int(cls)],
                        score=conf.item(),
                    ),
